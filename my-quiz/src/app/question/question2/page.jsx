    ...
    return (
        <div className='bg-zinc-800 w-[80vw] flex flex-col justify-center mt-10 gap-10 items-center h-fit p-8 rounded-md shadow-sm'>
            ...
            <div className='w-[80%] font-medium h-auto p-10 flex flex-wrap gap-5 bg-zinc-700 rounded-md shadow-sm items-center justify-center'>
                <Button url={`/question/finally`} answer={WrongAdd} cor={`bg-red-400`} alternativa={`Falo`} />
                <Button url={`/question/finally`} answer={WrongAdd} cor={`bg-purple-400`} alternativa={`Falava`} />
                <Button url={`/question/finally`} answer={CorrectAdd} cor={`bg-pink-400`} alternativa={` Falei`} />
                <Button url={`/question/finally`} answer={WrongAdd} cor={`bg-blue-400`} alternativa={`Falará`} />
            </div>
        </div>
    )
}